...
public class DatabaseChangelog {
    ...
    public void t(MongoDatabase db) {
        ...
        Document book1 = new Document("name", "Harry Potter and the Philosophers Stone")
                .append("author", Collections.singletonList(new DBRef("author", authorId1)))
                .append("genre", Collections.singletonList(new DBRef("genre", genreId1)))
                .append("comments", Collections.singletonMap(UUID.randomUUID().toString(), new Document("text", "Comment1")));
        books.insertOne(book1);

        Document book2 = new Document("name", "Wonnie-the-Pooh")
                .append("author", Collections.singletonList(new DBRef("author", authorId2)))
                .append("genre", Collections.singletonList(new DBRef("genre", genreId2)))
                .append("comments", Collections.singletonMap(UUID.randomUUID().toString(), new Document("text", "Comment2")));
        books.insertOne(book2);

        Document book3 = new Document("name", "Sherlock Holmes")
                .append("author", Collections.singletonList(new DBRef("author", authorId1)))
                .append("genre", Collections.singletonList(new DBRef("genre", genreId1)))
                .append("comments", Collections.singletonMap(UUID.randomUUID().toString(), new Document("text", "Comment3")));
